...
userSchema.pre('save', async function (next) {
  if (!this.isModified('password')) {
    // Checks if the password is already modified, you dont want to hash the password again if its already been hashed | Proceeds to the next lifecycle if its already hashed
    next();
  }
  // Password Hashing
...
userSchema.methods.getJWTSignedToken = function () {
  return jwt.sign({ id: this._id }, process.env.JWT_SECRET, {
    // Signs the id data based on the secret, that becomes an encoded token which is then returned to sendToken function | NOTE how the encoding corresponds to the id
